...
static void test_apply_mode_accepts_and_actuates() {
  ...
  TEST_ASSERT_TRUE(applyMode(c, "on", 0) == Outcome::Accepted);
...
static void test_apply_mode_rejects_garbage_and_leaves_controller_untouched() {
  ...
  TEST_ASSERT_TRUE(applyMode(c, "disco", 0) == Outcome::Rejected);
  TEST_ASSERT_TRUE(applyMode(c, "", 0) == Outcome::Rejected);
  TEST_ASSERT_TRUE(c.mode() == BlinkMode::ON);
}
...
static void test_apply_mode_same_value_is_unchanged() {
  ...
  TEST_ASSERT_TRUE(applyMode(c, "off", 0) == Outcome::Unchanged);
}

static void test_apply_mode_blink_baselines_without_immediate_toggle() {
  BlinkController c;
  c.setMode(BlinkMode::ON);  // lamp on
  TEST_ASSERT_TRUE(applyMode(c, "blink", 0) == Outcome::Accepted);
...
static void test_apply_period_accepts() {
  ...
  TEST_ASSERT_TRUE(applyPeriod(c, "1000", 0) == Outcome::Accepted);
  TEST_ASSERT_EQUAL_UINT32(1000, c.period());
}

static void test_apply_period_clamps_to_bounds() {
  BlinkController c;
  TEST_ASSERT_TRUE(applyPeriod(c, "1", 0) == Outcome::Accepted);
  TEST_ASSERT_EQUAL_UINT32(BlinkController::PERIOD_MIN_MS, c.period());

  TEST_ASSERT_TRUE(applyPeriod(c, "99999999", 0) == Outcome::Accepted);
...
static void test_apply_period_rejects_non_positive_and_non_numeric() {
  BlinkController c;
  TEST_ASSERT_TRUE(applyPeriod(c, "0", 0) == Outcome::Rejected);
  TEST_ASSERT_TRUE(applyPeriod(c, "-5", 0) == Outcome::Rejected);
  TEST_ASSERT_TRUE(applyPeriod(c, "", 0) == Outcome::Rejected);
  TEST_ASSERT_TRUE(applyPeriod(c, "abc", 0) == Outcome::Rejected);
  TEST_ASSERT_TRUE(applyPeriod(c, "50abc", 0) == Outcome::Rejected);  // trailing garbage
...
static void test_apply_period_same_value_is_unchanged() {
  BlinkController c;
  c.setPeriod(1000);
  TEST_ASSERT_TRUE(applyPeriod(c, "1000", 0) == Outcome::Unchanged);
}
